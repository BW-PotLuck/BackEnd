...
router.delete('/:id', async (req, res, next) => {
	try {
		await Events.remove(req.params.id).then((event) => {
			res
				.status(202)
				.json({
					message: `You have successfully removed event ${event.title}`,
					event,
				});
		});
	} catch (err) {
...
router.post('/:id/guests', validateEvent, async (req, res, next) => {
	...
	await Events.addGuest(req.params.id, newGuest)
		.then((guest) => {
			res
				.status(201)
				.json({
					message: `You have successfully added a guest to the event list.`,
					guest,
				});
		})
		.catch(next);
...
//       await Events.updateGuest( req.params.id, updateGuest)
//       .then((event) =>{
//           next({ message: `Event ${event.title} has been updated`, event });
//       })
	...
	await Events.addItem(req.params.id, req.body)
		.then((item) => {
			res
				.status(201)
				.json({
					message: `You have successfully added ${item.item_name} to the list`,
					item,
				});
		})
		.catch(next);
...
router.delete('/:id/items', async (req, res, next) => {
	try {
		if (!req.body.item_name) {
			...
			if (count !== 0) {
				await Events.findItemsByEvent(req.params.id).then((item) => {
					res
						.status(200)
						.json({
							message: `You have succesffully removed an item from the list`,
							item,
						});
				});
			} else {
